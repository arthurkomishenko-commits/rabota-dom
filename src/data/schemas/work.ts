...
export function workSchema({ image }: SchemaContext) {
  ...
  return z
    .object({
      ...
      /**
       * Непубликуемая запись для проверки самой схемы. Не рендерится нигде,
       * отсекается репозиторием и ловится `check:surface`.
       */
      fixture: z.boolean().default(false),

      /**
       * Демо-паспорт: правдоподобный, но выдуманный (бриф §13, DEC-0014).
       * В отличие от фикстуры ВИДЕН в staging — обязательно с маркером
       * «демо-данные», чтобы спутать с реальной работой было невозможно.
       * `check:freeze` не даст собрать релиз, пока хоть одна такая запись жива.
       */
      demo: z.boolean().default(false),

      /**
       * Рабочее название — оно есть в паспорте (`docs/INTAKE_TEMPLATE.md`),
       * но в первой версии схемы его не было, и карточка подставляла первое
       * предложение описания. Пробел найден на скриншоте витрины, BUG-0007.
       */
    ...
    /**
     * Демо-отзывов не существует (бриф §13). Выдуманный отзыв реального
     * человека — это не заглушка, а подделка; помеченная подделка остаётся
     * подделкой. Поэтому запрет машинный, а не «договорились не делать».
     */
    .refine((work) => !(work.demo && work.review), {
      message: 'у демо-записи не может быть отзыва: выдуманный отзыв — подделка, а не заглушка',
      path: ['review'],
    })
